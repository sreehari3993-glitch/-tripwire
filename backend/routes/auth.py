...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    mentor = db.query(Mentor).filter(Mentor.mentor_id == form_data.username).first()
    if not mentor or not pwd_context.verify(form_data.password, mentor.password_hash):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
